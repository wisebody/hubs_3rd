  ...
  tick = (function() {
    ...
    return function tick(scene, dt) {
      ...
      */
      ...
      if (window.APP.store.state.preferences.isThirdPerson === true) {
        this.nextMode();
        window.APP.store.state.preferences.isThirdPerson = false;
      }
      ...
      if (this.mode === CAMERA_MODE_FIRST_PERSON) {
        ...
        if (this.mode === CAMERA_MODE_THIRD_PERSON_NEAR) {
          translation.makeTranslation(0, 1.85, 0);
        } else {
        ...
        this.viewingCamera.object3DMap.camera.position.copy(new THREE.Vector3(0, 0, 1.85));
